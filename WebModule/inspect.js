...
var xPathFinder = new Inspector();

class highlight_by_xpath {
    constructor() {
      this.win = window;
      this.doc = window.document;
      this.xpath = [];
    ...
    highlight(xpath){
        ...
        {
            var element = document.evaluate(xpath[i],document).iterateNext();
            this.highlight_xpath(element,i);
        }
    }

    highlight_xpath(element,i){
        var delay_time = 1400;
        window.setTimeout(function(){
            element.style.boxShadow = "rgb(255,0,0) 0px 0px 6px 6px";
        },delay_time*i+200);
        window.setTimeout(function(){
...
var highlight = new highlight_by_xpath();
